...
const RangeDatePicker = ({ style, isOpened, setOpened, onRangeChanged }: IProps) => {
    ...
    const getMonthDayCSSClass = (date: Dayjs) => {
        let cssClass = "range-date-picker__month-day-list__month-day";

        // If no days have been selected, all the days are start-day candidates
        if (startDate == null && endDate == null) {
            cssClass += " range-date-picker__month-day-list__month-day--start-day-candidate";
        }
        ...
        if (startDate != null) {
            ...
            if (isDateInCandidateRange || isDateInRange) {
                cssClass += " range-date-picker__month-day-list__month-day--selected-range-day";
            }
            ...
            if (isDateTheStartDate || isDateTheEndDateCandidate || isDateTheEndDate) {
                cssClass += " range-date-picker__month-day-list__month-day--selected-range-extreme-day";
            }
        }
    ...
    return (
        <div ref={datePickerFrame} className={"range-date-picker range-date-picker--" + (isOpened ? "opened" : "closed")} style={style}>
            <div className="range-date-picker__header">
                <MiniRoundedIconButton Icon={MdChevronLeft} clickEvent={selectPreviousMonth}/>
                <p
                    className="range-date-picker__header__month-name paragraph--sub-title paragraph--small">
            ...
            <div className="range-date-picker__content">
                <div className="range-date-picker__weekday-list">
                    {
                        dayNames.map(dayName => {
                            return (
                                <p key={dayName} className="paragraph--small paragraph--bold range-date-picker__weekday-list__weekday">
                                    {dayName}
                                </p>
                            );
                        })
                ...
                <div className="range-date-picker__month-day-list">
                    {
                        [...Array(dayjs(selectedYearAndMonth).daysInMonth()).keys()].map(day => {
                            ...
                            return (
                                <div
                                    ...
                                    <p className="paragraph--small paragraph--bold range-date-picker__month-day-list__month-day__text">
                                        {day + 1}
                                    </p>
                                </div>
                            );
