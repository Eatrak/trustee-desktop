...
const MiniSelect = forwardRef<IHandle, IProps>(
    (
        ...
        const renderOptions = () => {
            return options.map((option) => {
                return (
                    <div
                        key={option.name}
                        className={`mini-select__option ${
                            option.name == selectedOption?.name
                                ? "mini-select__option--selected"
                                : ""
                        }`}
                        onClick={() => selectOption(option)}
                    >
                        <p className="mini-select__option_text paragraph--small">
                            {option.name}
                        </p>
                    </div>
                );
        ...
        const closeSelectWhenTouchingOutsideEvent = (e: MouseEvent) => {
            ...
            const haveSelectChildsBeenClicked = selectFrame.current?.contains(
        ...
        useEffect(
            () => () => {
                document.removeEventListener(
                    "mousedown",
                    closeSelectWhenTouchingOutsideEvent,
                );
            },
            [],
        );
        ...
        return (
            <div
                ...
                className={
                    "mini-select " +
                    (errors.length > 0 ? "mini-select--error " : "") +
                    (className || "")
                }
                tabIndex={0}
            >
                <div
                    className="mini-select__body"
                    onTimeUpdate={() => switchPanelStatus()}
                    onClick={() => switchPanelStatus()}
                >
                    <p className="paragraph--small mini-select__body__text">
                        {selectedOption?.name}
                    </p>
                </div>
                <div
                    className={
                        "mini-select__options-panel mini-select__options-panel--" +
                        (opened ? "opened" : "closed") +
                        (hasNeverBeenOpened
                            ? " select__options-panel--has-never-been-opened"
                            : "")
                    }
                >
