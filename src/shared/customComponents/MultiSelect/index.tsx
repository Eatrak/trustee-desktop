...
interface IProps {
    ...
    selectedOptions: MultiSelectOptionProprieties[];
    setSelectedOptions: (newSelectedOptions: MultiSelectOptionProprieties[]) => any;
}
...
const MultiSelect = ({
    ...
    const renderOptions = () => {
        let checks: { [optionName: string]: string } = {};
            ...
            return filteredOptions.map((option) => {
                return (
                    <MultiSelectOption
                        key={option.name}
                        option={option}
                        isChecked={checks[option.name] != undefined}
    ...
    return (
        <div
            ...
            className={
                "multi-select border border-input rounded-md text-sm " + (className || "")
            }
            tabIndex={0}
        >
